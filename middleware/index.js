...
app.post('/recovery', async (req, res) => {
  ...
  try {
  const recovery_modules = await RecoveryModule.find({assigned: false, chainId: req.body.chainId});  
  ...
  console.log(recovery_modules)

  const account = new RecoveryAccount({recoveryEmailHash: req.body.recoveryEmailHash, safeAddress: req.body.safeAddress, recoveryModuleAddress: recovery_modules[0].recoveryModuleAddress, chainId: req.body.chainId});
  await account.save();
